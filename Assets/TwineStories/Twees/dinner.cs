/*
Generated by UnityTwine on 4/25/2016 10:11:43 AM
...
public class dinner: TwineStory
{
	...
	IEnumerable<TwineOutput> passageExecute_8()
	{
		...
		yield return new TwineText(@"");
		yield return new TwineText(@"<i>As expected, her conversation is dry and acerbic--at least, it is at the start of the night. As the courses progress, you find that her comments have lost their biting edge. Maybe it's just the wine that loosens the tight corners of her mouth, but you dare to think it might actually be your company.</i>");
		yield return new TwineText(@"");
	...
	IEnumerable<TwineOutput> passageExecute_10()
	{
		...
		yield return new TwineText(@"");
		yield return new TwineText(@"<i>Pat always seems weighed down by something, distanced from the present by a problem no one else knows about. But tonight, bit by bit, her shoulders lift, and her expression lightens. When Beauregard's fancifully prepared dessert course makes her clap her hands and laugh delightedly, she looks years younger.</i>");
		yield return new TwineText(@"");
	...
	IEnumerable<TwineOutput> passageExecute_11()
	{
		...
		yield return new TwineText(@"");
		yield return new TwineText(@"<i>Eating with your ungainly claws becomes a source of laughter rather than embarrassment. Your struggle with a spoonful of soup makes both ends of the table turn red in the face from laughing--not that your skin is really visible under all the fur.</i>");
		yield return new TwineText(@"");
